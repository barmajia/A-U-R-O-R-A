...
class SellerDB extends ChangeNotifier {
  ...
  Future<void> init() async {
    // Removed password column for security; added latitude/longitude.
    db.execute('''
      CREATE TABLE IF NOT EXISTS $tableName (
        ...
        latitude REAL,          -- NEW: location for proximity
        longitude REAL,         -- NEW: location for proximity
        last_login TEXT,        -- NEW: optional last login timestamp
        created_at TEXT,
        updated_at TEXT
      );
    ''');
  ...
  /// Add seller to local database (removed password)
    ...
    try {
      ...
      final stmt = db.prepare('''
        INSERT INTO $tableName (
          ...
          currency, account_type, is_verified, latitude, longitude, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
      ''');
      ...
      stmt.execute([
        ...
        seller['latitude'], // may be null
        seller['longitude'], // may be null
  ...
  Future<void> updateSeller(String userId, Map<String, dynamic> data) async {
    try {
      ...
      setFields.add('updated_at = ?');
      values.add(DateTime.now().toIso8601String());

      if (setFields.isEmpty) return; // Nothing to update
  ...
  Future<void> updateLocation(
    ...
    try {
      db.execute(
        'UPDATE $tableName SET latitude = ?, longitude = ? WHERE user_id = ?',
